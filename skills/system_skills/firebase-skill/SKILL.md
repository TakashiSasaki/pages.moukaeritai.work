...
1.  **Provision Firebase (MANDATORY)**:

    Before writing ANY Firebase integration code, you MUST configure Firebase.
    The Firebase setup process always follows these steps:

    1.  Call `set_up_firebase` with `userConfirmedTermsAcceptedInUI: false` and
        the required platform (`web` or `android`).
    2.  **STOP** the conversation immediately and wait for the user's
        confirmation in the chat (e.g., "Firebase terms accepted, continue...").
    3.  **Critical Resuming Behavior**: When you are resumed after the user
        accepts/confirms, your VERY FIRST ACTION in that turn must be to call
        `set_up_firebase` with `userConfirmedTermsAcceptedInUI: true` (along
        with the required platform and any user-specified `firestoreRegion` or
        `databaseId` if provided).

        > [!CAUTION] You MUST call the `set_up_firebase` tool again with
        > `userConfirmedTermsAcceptedInUI: true` before performing any other
        > task (such as writing integration code, modifying files, building, or
        > linting). If you skip this tool call, the database will not be
        > provisioned and the application will fail.
...
When a Firestore operation fails due
