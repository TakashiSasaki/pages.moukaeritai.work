...
- **Renewed Development Line ($v1.7.x$)**: The `v1.7.x` development run introduces the server-side icon resolver, the Firestore-cached Launcher site icon cache system, client-side caching with optimistic fallback rendering, and expanded automated testing.
  ...
  - **Milestone 1.7.34**: Release Candidate Documentation Polish & Go/No-Go Gate (Current active ready milestone).
...
- **Primary deployment target**: **Cloud Run** (Active, Live).
  ...
  - Current Major Milestone: Release Candidate Documentation Polish & Go/No-Go Gate (1.7.34)
...
- Advanced primary build threshold to `1.7.34 (Release Candidate Documentation Polish & Go/No-Go Gate)` with patch version governance and public no-auth E2E validation controls.
